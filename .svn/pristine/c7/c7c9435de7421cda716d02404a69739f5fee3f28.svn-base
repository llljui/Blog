...
/**获取配件系统上级分类**/
function get_car_system_parent_cats($cat)
{
    if ($cat == 0)
    {
        return array();
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['ecs']->table('car_system_category'));
   ...
   foreach ($res AS $row)
    {
        switch ($row['type'])
        {
            case 1:
                /* 分类下的商品 */
                //edited by wuzeguo 
                $GLOBALS['smarty']->assign('goods_cat_' . $row['id'], assign_car_system_cat_goods($row['id'], $row['number']));
            break;
            case 2:
                /* 品牌的商品 */
