 ...
 * ============================================================================
 * $Author: Shadow & 小滨汽配
 * $Id: lib_main.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
*/
...
function admin_priv($priv_str, $msg_type = '' , $msg_output = true)
{
    ...
    if ($_SESSION['dept_action_list'] == 'all')
    {
        return true;
    }

    if (strpos(',' . $_SESSION['dept_action_list'] . ',', ',' . $priv_str . ',') === false)
    {
...
{
    return (preg_match('/,*'.$authz.',*/', $_SESSION['dept_action_list']) || $_SESSION['dept_action_list'] == 'all');
}
...
function get_bonus_type()
{
    ...
    $sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS['ecs']->table('bonus_type') .
           ' WHERE send_type = 3 AND dept_id='.$_SESSION['dept_id'];
...
function get_goods_list($filter)
{
    ...
    $suppid = (isset($_GET['suppId']) && intval($_GET['suppId'])>0) ? intval($_GET['suppId']) : $_SESSION['dept_id'];
    
    $where .= ' AND sgc.dept_id='.$suppid.' group by g.goods_id ';
   ...
   // $sql = 'SELECT goods_id, goods_name, shop_price '.
    ...
    $sql = 'select g.goods_id,g.goods_name,g.shop_price '.
           'from ' . $GLOBALS['ecs']->table('dept_goods_cat') . 'as sgc '.
...
function get_where_sql($filter)
{
    ...
    $where .= isset($filter->cat_id) && $filter->cat_id > 0 ? ' AND ' . get_children_dept($filter->cat_id) : '';
    ...
    $where .= isset($filter->depts_id) && trim($filter->depts_id) != '' ?
        " AND (g.depts_id = '" . $filter->depts_id . "') " : '';
 ...
 */
 ...
 //入驻商后台添加分类做判断
function cat_exists_dept($suppid,$cat_name, $parent_cat, $exclude = 0)
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('dept_category').
    " WHERE dept_id=".$suppid." AND parent_id = '$parent_cat' AND cat_name = '$cat_name' AND cat_id<>'$exclude'";
 ...
 * @return      array
 */
function depts_list_info($conditions = '')
{
    $where = '';
    if (!empty($conditions))
    {
    ...
    /* 查询 */
    $sql = "SELECT depts_id, depts_name, depts_desc
            FROM " . $GLOBALS['ecs']->table("depts") . "
            $where";
 ...
 * @return  array
 */
function depts_list_name()
{
    /* 查询 */
    $depts_list = depts_list_info(' is_check = 1 ');

    /* 供货商名字 */
    $depts_name = array();
    if (count($depts_list) > 0)
    {
        foreach ($depts_list as $depts)
        {
            $depts_name[$depts['depts_id']] = $depts['depts_name'];
        }
    }
...
function cat_list_2($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
            ...
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['ecs']->table('dept_category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('dept_category') . " AS s ON s.parent_id=c.cat_id  where c.dept_id = ".$_SESSION['dept_id'].
                " GROUP BY c.cat_id ".
            ...
            /*$sql = "SELECT cat_id, COUNT(*) AS goods_num " .
                    ...
                    " WHERE dept_id = ".$_SESSION['dept_id']." AND is_delete = 0 AND is_on_sale = 1 " .
                    " GROUP BY cat_id";
            $res2 = $GLOBALS['db']->getAll($sql);*/

            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['ecs']->table('dept_goods_cat') . " AS gc , " . $GLOBALS['ecs']->table('goods') . " AS g " .
                    " WHERE gc.dept_id = ".$_SESSION['dept_id']." AND g.goods_id = gc.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 " .
                    " GROUP BY gc.cat_id";
    ...
    else
    {
        ...
        {
            $options[$key]['url'] = build_uri('dept', array('go'=>'category','suppid'=>$_SESSION['dept_id'],'cid' => $value['cat_id']), $value['cat_name']);
        }
...
function create_shop_settiongs()
{
	...
	if(!isset($_SESSION['dept_id']) || intval($_SESSION['dept_id'])<=0){
		return;
	}
	
	$sql = "SELECT count(id) FROM " . $ecs->table('dept_config') ." WHERE dept_id=".$_SESSION['dept_id'];
	$num = $db->getOne($sql);
	if($num>0){
		return;
	}else{
		$insql = "INSERT INTO ". $ecs->table('dept_config') ." (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `dept_id`) VALUES
				(1, 0, 'shop_info', 'group', '', '', '', 1, ".$_SESSION['dept_id']."),
				(2, 0, 'hidden', 'hidden', '', '', '', 1, ".$_SESSION['dept_id']."),
				(8, 0, 'sms', 'group', '', '', '', 1, ".$_SESSION['dept_id']."),
				(101, 1, 'shop_name', 'text', '', '', '商家店铺名称', 1, ".$_SESSION['dept_id']."),
				(102, 1, 'shop_title', 'text', '', '', '商家店铺标题', 1, ".$_SESSION['dept_id']."),
				(103, 1, 'shop_desc', 'hidden', '', '', '商家店铺描述', 1, ".$_SESSION['dept_id']."),
				(104, 1, 'shop_keywords', 'text', '', '', '商家店铺关键字', 1, ".$_SESSION['dept_id']."),
				(105, 1, 'shop_country', 'manual', '', '', '1', 1, ".$_SESSION['dept_id']."),
				(106, 1, 'shop_province', 'manual', '', '', '0', 1, ".$_SESSION['dept_id']."),
				(107, 1, 'shop_city', 'manual', '', '', '0', 1, ".$_SESSION['dept_id']."),
				(108, 1, 'shop_address', 'text', '', '', '', 1, ".$_SESSION['dept_id']."),
				(109, 1, 'qq', 'text', '', '', '', 1, ".$_SESSION['dept_id']."),
				(110, 1, 'ww', 'text', '', '', '', 1, ".$_SESSION['dept_id']."),
				(111, 1, 'skype', 'hidden', '', '', '', 1, ".$_SESSION['dept_id']."),
				(112, 1, 'ym', 'hidden', '', '', '', 1, ".$_SESSION['dept_id']."),
				(113, 1, 'msn', 'hidden', '', '', '', 1, ".$_SESSION['dept_id']."),
				(114, 1, 'service_email', 'text', '', '', '', 1, ".$_SESSION['dept_id']."),
				(115, 1, 'service_phone', 'text', '', '', '', 1, ".$_SESSION['dept_id']."),
				(116, 1, 'shop_closed', 'select', '0,1', '', '0', 1, ".$_SESSION['dept_id']."),
				(117, 1, 'close_comment', 'textarea', '', '', '该店铺正在装修', 1, ".$_SESSION['dept_id']."),
				(118, 1, 'shop_logo', 'file', '', '../themes/".'{$template}'."/images/', '', 1, ".$_SESSION['dept_id']."),
				(119, 1, 'licensed', 'hidden', '0,1', '', '1', 1, ".$_SESSION['dept_id']."),
				(120, 1, 'user_notice', 'hidden', '', '', '用户中心公告！', 1, ".$_SESSION['dept_id']."),
				(121, 1, 'shop_notice', 'textarea', '', '', '商家店铺介绍:欢迎光临手机网,我们的宗旨：诚信经营、服务客户！\r\n<MARQUEE onmouseover=this.stop() onmouseout=this.start() \r\nscrollAmount=3><U><FONT color=red>\r\n<P>咨询电话: 400-888-8888</P></FONT></U></MARQUEE>', 1, ".$_SESSION['dept_id']."),
				(122, 1, 'shop_reg_closed', 'hidden', '1,0', '', '0', 1, ".$_SESSION['dept_id']."),
				(123, 1, 'shop_index_num', 'textarea', '', '', '8\r\n6\r\n4', 1, ".$_SESSION['dept_id']."),
				(124, 1, 'shop_search_price', 'textarea', '', '', '0-1000元\r\n1000-2000元\r\n2000-4000元', 1, ".$_SESSION['dept_id']."),
				(201, 2, 'shop_header_color', 'hidden', '', '', '#E4368F', 1, ".$_SESSION['dept_id']."),
				(202, 2, 'shop_header_text', 'hidden', '', '', '', 1, ".$_SESSION['dept_id']."),
				(203, 2, 'template', 'hidden', '', '', 'dianpu1', 1, ".$_SESSION['dept_id']."),
				(204, 2, 'stylename', 'hidden', '', '', '', 1, ".$_SESSION['dept_id']."),
				(205, 2, 'flash_theme', 'hidden', '', '', '".$_SESSION['dept_name'].$_SESSION['dept_id']."', 1, ".$_SESSION['dept_id']."),
				(801, 8, 'sms_shop_mobile', 'text', '', '', '', 1, ".$_SESSION['dept_id']."),
				(802, 8, 'sms_order_placed', 'select', '1,0', '', '0', 0, ".$_SESSION['dept_id']."),
				(803, 8, 'sms_order_payed', 'select', '1,0', '', '0', 1, ".$_SESSION['dept_id']."),
				(804, 8, 'sms_order_shipped', 'select', '1,0', '', '0', 1, ".$_SESSION['dept_id'].");";
