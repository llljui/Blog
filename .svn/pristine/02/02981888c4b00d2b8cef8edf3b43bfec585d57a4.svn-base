...
if (!$smarty->is_cached('car_system_catalog.dwt'))
{
    /* 取出所有分类 */
    $cat_list = car_system_cat_list(0, 0, false);
    ...
    assign_template();
    assign_dynamic('car_system_catalog');
    $position = assign_ur_here(0, $_LANG['catalog']);
    $smarty->assign('page_title', $position['title']);   // 页面标题
    $smarty->assign('ur_here',    $position['ur_here']); // 当前位置
    $smarty->assign('categories_all',      get_categories_tree()); // 分类树

    $smarty->assign('car_system_categories_all',      get_car_system_categories_tree()); // 分类树
...
$smarty->display('car_system_catalog.dwt');
