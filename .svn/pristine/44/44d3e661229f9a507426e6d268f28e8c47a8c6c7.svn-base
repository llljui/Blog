 ...
 * ============================================================================
 * $Author: Shadow & 小滨汽配
 * $Id: goods.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
*/
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    //仓库列表
    $dept_list = $GLOBALS['db']->getAll('select * from ' . $GLOBALS['ecs']->table('dept') . ' where status = 1');
   ...
   create_html_editor('goods_desc', htmlspecialchars($goods['goods_desc'])); /* 修改 by bbs.hongyuvip.com 百度编辑器 */
    ...
    $smarty->assign('goods_dept_ids', explode(',', $goods['dept_ids']));
    ...
    $system_cat_list = car_system_cat_list(0, $selected, false);
	...
	$smarty->assign('goods_system_cat_list', cat_list_to_json($system_cat_list, $goods['system_cat_id']));
	$smarty->assign('goods_system_cat_name', $system_cat_list[$goods['system_cat_id']]['cat_name']);
	$smarty->assign('goods_system_cat_id', $goods['system_cat_id']);
	// 代码增加_start_derek20150129admin_goods  bbs.hongyuvip.com
    ...
    //添加年份,排量，是否原配
    $year_output = get_year_output();
    $smarty->assign('year_list', $year_output['year_list']);
    $smarty->assign('output_list', $year_output['output_list']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    //仓库
    $dept_ids = trim($_REQUEST['dept_ids']);
    $dept_ids = substr($dept_ids, 0, -1);
    //dump($dept_ids);die;
    //$dept_arr = explode(',', $dept_ids);
    ...
    $system_catgory_id = empty($_POST['system_cat_id']) ? '' : intval($_POST['system_cat_id']);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, cost_price, year, is_original, output, system_cat_id, dept_ids)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$cost_price', '$_POST[year]', '$_POST[is_original]', '$_POST[output]', '$system_catgory_id', '$dept_ids')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, cost_price, year, is_original, output, system_cat_id, dept_ids)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$cost_price', '$year', '$is_original', '$output', '$system_catgory_id', '$dept_ids')";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "goods_sn = '$goods_sn', " .
                "cat_id = '$catgory_id', " .
                "system_cat_id = '$system_catgory_id', " .
                "brand_id = '$brand_id', " .
...
elseif ($_REQUEST['act'] == 'ajax_category')
{
	...
	print $json;
//added by wuzeguo 修改成为车系统分类
} elseif ($_REQUEST['act'] == 'ajax_system_category') {
    
	$cat_list = car_system_cat_list(0, $selected, false);
