 ...
 * ============================================================================
 * $Author: Shadow & 小滨汽配
 * $Id: init.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
*/
...
require(ROOT_PATH . 'includes/lib_dept_common.php');
...
/* 载入系统参数 */
$_CFG = array_merge(load_config(),load_config_dept());
...
if (!file_exists('../temp/compiled/dept'))
{
    @mkdir('../temp/compiled/dept', 0777);
    @chmod('../temp/compiled/dept', 0777);
}
...
$smarty->compile_dir   = ROOT_PATH . 'temp/compiled/dept';
if ((DEBUG_MODE & 2) == 2)
{
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['ECSCP']['dept_id']) && !empty($_COOKIE['ECSCP']['dept_user_id']) && !empty($_COOKIE['ECSCP']['dept_pass']))
    {
        // 找到了cookie, 验证cookie信息
        //$sql = 'SELECT u.user_id, u.user_name, u.password,  u.last_login, s.dept_id ' .
        //        ' FROM ' .$ecs->table('users') . " AS u ,  ". $ecs->table('dept') ." AS s ".
        //        " WHERE u.user_id =s.user_id and s.dept_id= '" . intval($_COOKIE['ECSCP']['dept_id']) . "'";
        //$row = $db->GetRow($sql);
        $sql = 'SELECT user_id, user_name, password, action_list, last_login, dept_id ' .
                ' FROM ' .$ecs->table('dept_admin_user') .
                " WHERE user_id = '" . intval($_COOKIE['ECSCP']['dept_user_id']) . "' AND dept_id=".intval($_COOKIE['ECSCP']['dept_id']);
        ...
        {
            // 没有找到这个记录
            setcookie($_COOKIE['ECSCP']['dept_id'],   '', 1);
            setcookie($_COOKIE['ECSCP']['dept_pass'], '', 1);
        ...
        {
            // 检查密码是否正确
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['dept_pass'])
            {
                ...
                $db->query('UPDATE ' . $ecs->table('dept_admin_user') .
                            " SET last_login = '" . gmtime() . "', last_ip = '" . real_ip() . "'" .
                            " WHERE user_id = '" . $_SESSION['dept_user_id'] . "'");
				
            }
            else
            {
                setcookie($_COOKIE['ECSCP']['dept_id'],   '', 1);
				setcookie($_COOKIE['ECSCP']['dept_user_id'],   '', 1);
                setcookie($_COOKIE['ECSCP']['dept_pass'], '', 1);
