...
{//类定义开始
    ...
    static function save($type=self::FILE,$destination='',$extra='')
    {
        	...
        	if(!is_dir( ROOT_PATH."logs".$destination."/"))
        	{
        		if(!mkdir(ROOT_PATH."logs".$destination."/"))
        		{
        			return false;
        		}
        	}
        	
            $destination = ROOT_PATH."logs".$destination."/".date('Y-m-d').".log";
    ...
    static function write($message,$level=self::DEBUG,$type=self::FILE,$destination='',$extra='')
    {
        	...
        	if(!is_dir( ROOT_PATH."logs/".$destination."/"))
        	{
        		if(!mkdir(ROOT_PATH."logs/".$destination."/", 0777))
        		{
        			return false;
        		}
        	}
        	
            $destination = ROOT_PATH."logs/".date('Y-m-d').".log";
