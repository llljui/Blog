...
/* 代码增加_start  BY  www.xiaobinqipei.com */
...
/* 代码增加_end  BY  www.xiaobinqipei.com */
...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	//www.xiaobinqipei.com start add 2015-3-26
	$time_xg_now=gmtime();
	...
	//www.xiaobinqipei.com end add 2015-3-26
	...
	$result['goods_id'] = $goods->goods_id;
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加 www.xiaobinqipei.com
...
/* 代码增加_end  By www.xiaobinqipei.com */
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
				/* 四合一修改 start by www.xiaobinqipei.com */
                ecs_header("Location: flow.php?step=checkout\n");
				/* 四合一修改 end by www.xiaobinqipei.com */
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  www.xiaobinqipei.com  注释或删掉  */
        //ecs_header("Location: flow.php?step=consignee\n");   
        //exit;
		/* 代码修改_end By www.xiaobinqipei.com */
    }
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
		...
		/*余额额支付密码_添加_START_www.xiaobinqipei.com*/
        $order['pay_code'] = $payment_info['pay_code'];
	    /*余额额支付密码_添加_END_www.xiaobinqipei.com*/
...
{	
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
	        ...
	        /*增值税发票_删除_START_www.xiaobinqipei.com*/
        	//'inv_type'        => $_POST['inv_type'],
        	//'inv_payee'       => trim($_POST['inv_payee']),
        	//'inv_content'     => $_POST['inv_content'],
			/*增值税发票_删除_END_www.xiaobinqipei.com*/
	...
	//$GLOBALS['db'] -> query("unlock tables");
	    ...
	    if ($order['order_amount'] <= 0)
	    {
	        ...
	        foreach ($res AS $row)
	        {   
                    /* 代码增加_start  By www.xiaobinqipei.com _sunlizhi*/
...
//                    }
                    ...
                    /* 代码增加_end  By www.xiaobinqipei.com _sunlizhi*/
...
/* 代码修改_start  By  www.xiaobinqipei.com */
if ($_REQUEST['step'] == 'cart')
{
...
{
   /* 代码修改_start  By  www.xiaobinqipei.com  将这块替换掉*/
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加 www.xiaobinqipei.com
