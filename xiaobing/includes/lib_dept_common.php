 ...
 * 获取本店铺商品数量
 */
function get_dept_goods_count($suppid=0){
	
	$suppid = (intval($suppid)>0) ? intval($suppid) : intval($_GET['suppId']);
	$sql="SELECT count(`goods_id`) FROM ".$GLOBALS['ecs']->table('goods')." as g WHERE  g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.dept_id='$suppid'";
   return $GLOBALS['db']->getOne($sql);
}

function assign_template_dept($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('username',        htmlspecialchars($suppinfo['dept_name']));//商家名称
    ...
    $smarty->assign('goodsnum',      get_dept_goods_count());//商家商品数量
    ...
    $smarty->assign('navigator_list_dept',        get_navigator_dept($ctype, $catlist));  //自定义导航栏
    
    $smarty->assign('top_goods',       get_top10_dept());           // 销售排行
 ...
 * @return  string
 */
function add_style_dept($goods_name, $style)
{
 ...
 * @return  array         列表
 */
function get_navigator_dept($ctype = '', $catlist = array())
{
    $sql = 'SELECT * FROM '. $GLOBALS['ecs']->table('dept_nav') . '
            WHERE ifshow = \'1\' AND dept_id='.$_GET['suppId'].' ORDER BY type, vieworder';
 ...
 * @return  array
 */
function load_config_dept()
{
    $arr = array();
    
    $suppid = (isset($_GET['suppId'])&& intval($_GET['suppId'])>0) ? intval($_GET['suppId']) : (isset($_SESSION['dept_id']) ? $_SESSION['dept_id'] : 0);
    if($suppid <= 0){
    	return $arr;
    }

    $data = read_static_cache('dept_config'.$suppid);
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['ecs']->table('dept_config') . ' WHERE parent_id > 0 AND dept_id='.$suppid;
        ...
        */
        ...
        //}
        write_static_cache('dept_shop_config'.$suppid, $arr);
    }
    else
    {
        $arr = $data;
 ...
 * @return  array
 */
function get_categories_tree_dept($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('dept_category') . " WHERE cat_id = '$cat_id' AND dept_id=".$_GET['suppId'];
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('dept_category') . " WHERE parent_id = '$parent_id' AND is_show = 1 AND dept_id=".$_GET['suppId'];
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('dept_category') .
                "WHERE parent_id = '$parent_id' AND is_show = 1 AND dept_id=".$_GET['suppId']." ORDER BY sort_order ASC, cat_id ASC";
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $cat_arr[$row['cat_id']]['url']  = build_uri('dept', array('go'=>'category','suppid'=>$_GET['suppId'],'cid' => $row['cat_id']), $row['cat_name']);

                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_tree_dept($row['cat_id']);
                }
            }
        }
    }
...
function get_child_tree_dept($tree_id = 0)
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('dept_category') . " WHERE parent_id = '$tree_id' AND is_show = 1 AND dept_id=".$_GET['suppId'];
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('dept_category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 AND dept_id=".$_GET['suppId']." ORDER BY sort_order ASC, cat_id ASC";
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
               ...
               $three_arr[$row['cat_id']]['url']  = build_uri('dept', array('go'=>'category','suppid'=>$_GET['suppId'],'cid' => $row['cat_id']), $row['cat_name']);

               if (isset($row['cat_id']) != NULL)
                   {
                       $three_arr[$row['cat_id']]['cat_id'] = get_child_tree_dept($row['cat_id']);
 ...
 * @return  string
 */
function get_children_dept($cat = 0,$attr='sgc')
{
    return $attr.'.cat_id ' . db_create_in(array_unique(array_merge(array($cat), array_keys(cat_list_dept($cat, 0, false)))));
}
 ...
 * @return  mix
 */
function cat_list_dept($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;
    ...
    if ($res === NULL)
    {
            ...
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                ...
                "LEFT JOIN " . $GLOBALS['ecs']->table('dept_category') . " AS s ON s.parent_id=c.cat_id  where c.dept_id = ".$suppid.
                " GROUP BY c.cat_id ".
            ...
            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    ...
                    " WHERE gc.dept_id = ".$suppid." AND g.goods_id = gc.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 " .
                    " GROUP BY gc.cat_id";
    ...
    $options = cat_options_dept($cat_id, $res); // 获得指定分类下的子分类的数组
 ...
 * @return  void
 */
function cat_options_dept($spec_cat_id, $arr)
{
 ...
 * @return  array
 */
function get_top10_dept($cats = '')
{
	$suppid = (isset($_GET['suppId']) && intval($_GET['suppId'])>0) ? intval($_GET['suppId']) : $_SESSION['dept_id'];
	
    $cats = get_children($cats);
    $where = "AND g.dept_id=".$suppid;
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.market_price, g.original_img, g.goods_thumb, SUM(og.goods_number) as goods_number, g.dept_id ' .
 ...
 * @return  string
 */
function get_extension_goods_dept($cats)
{
    $extension_goods_array = '';
    $suppid = (isset($_GET['suppId']) && intval($_GET['suppId'])>0) ? intval($_GET['suppId']) : $_SESSION['dept_id'];
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['ecs']->table('dept_goods_cat') . " AS sgc WHERE $cats";
