...
function save_consignee($consignee, $default=false)
{
    ...
    $province_name = $GLOBALS['db']->getOne($re_sql . $consignee['province']);
    $city_name = $GLOBALS['db']->getOne($re_sql . $consignee['city']);
    $area_name = $GLOBALS['db']->getOne($re_sql . $consignee['district']);
    $all_address = $province_name . $city_name . $area_name . $consignee['address'];
    $point = getPointByAddress($all_address);
    $consignee['lng'] = $point['lng'];
    $consignee['lat'] = $point['lat'];
