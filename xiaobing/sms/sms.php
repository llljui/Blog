...
function sendSMS($mobile_phone, $content, $is_ali = false)
{
    if ($is_ali) { 
        ...
        //added by wuzeguo 
        include_once 'ChuanglanSms.php';
        $sms_obj = new ChuanglanSms();
        $str = sprintf($content[0], $content[1]['code'], $content[1]['product']);
        log_write($str, 'register');
        log_write($mobile_phone);
        //$sign = '【小滨汽配】';
        $hy_result = $sms_obj->sendSms($mobile_phone, $sign.$str);
    }
