 ...
 * ============================================================================
 * $Author: Shadow & 小滨汽配
 * $Id: car_system_category.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
*/
...
elseif (isset($_REQUEST['car_system_category']))
{
    $cat_id = intval($_REQUEST['car_system_category']);
}
else
{
    /* 如果分类ID为0，则返回首页 */
...
// 读取频道页模版
$index_file_sql = "select * from " . $ecs->table('car_system_category') . " where cat_id = " . $cat_id;
$index_file = $db->getRow($index_file_sql);
$is_index = $index_file['car_system_category_index'];
$is_index_dwt = $index_file['car_system_category_index_dwt'];
$index_dwt_file = $index_file['index_dwt_file'];
if (!$is_index)
{
	$index_dwt_files = 'car_system_category.dwt';
}
else
{
	$index_dwt_files = (($is_index_dwt == 0 || $index_dwt_file == '') ? 'car_system_category_index.dwt' : $index_dwt_file);
}
	/* 初始化分页信息 */
	...
	if (!$smarty->is_cached($index_dwt_files, $cache_id))
	{
		...
		$children = get_car_system_children($cat_id);

		$cat = get_car_system_cat_info($cat_id);   // 获得分类的相关信息

		if (!empty($cat))
		{
			$cat['style'] = empty($cat['style']) ? "car_system_category.css" : $cat['style'];
		...
		{
			$cat['grade'] = get_car_system_parent_grade($cat_id); //如果当前分类级别为空，取最近的上级分类
		}
		...
		if ($cat['grade'] > 1)
		{
			...
			*/
			...
			$sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max ".
				   ...
				   " WHERE ($children OR " . get_car_system_extension_goods($children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  ';
				   //获得当前分类下商品价格的最大值、最小值
			...
			foreach ($price_grade as $key=>$val)
			{
				...
				$price_grade[$temp_key]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_grade[$temp_key]['start'], 'price_max'=> $price_grade[$temp_key]['end'], 'filter_attr'=>$filter_attr_str, 'filter'=>$filter), $cat['cat_name']);
			...
			$price_grade[0]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter), $cat['cat_name']);
		...
		$sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ".
				"FROM " . $GLOBALS['ecs']->table('brand') . "AS b LEFT JOIN ".
					$GLOBALS['ecs']->table('goods') . " AS g ON b.brand_id = g.brand_id" .
				" WHERE (" .$children ."OR " . 'g.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(car_system_cat_list($cat_id, 0, false))))) . ") AND b.is_show = 1 " .
		...
		foreach ($brands AS $key => $val)
		{
			...
			$brands[$temp_key]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => $val['brand_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter), $cat['cat_name']);
		...
		if ($brand)
		{
				$condition[] = array(
					...
					'cond_url' => build_uri('car_system_category', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter), $cat['cat_name']) ,
				);
		}
		...
		if ($price_min || $price_max)
		{
				$condition[] =array(
						...
						'cond_url' => build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter), $cat['cat_name'])
					); 
		}
		$smarty->assign('brand_zimu_68ecshop', $brand_zimu);
		$smarty->assign('url_no_price', build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand,  'filter_attr'=>$filter_attr_str, 'filter'=>$filter), $cat['cat_name']));

		$brands[0]['brand_name'] = $_LANG['all_attribute'];
		$brands[0]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter), $cat['cat_name']);
		...
		if ($cat['filter_attr'] > 0)
		{
			...
			{
				$sql = "SELECT a.attr_name FROM " . $ecs->table('attribute') . " AS a, " . $ecs->table('goods_attr') . " AS ga, " . $ecs->table('goods') . " AS g WHERE ($children OR " . get_car_system_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
				if($temp_name = $db->getOne($sql))
				{
					...
					$sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
						   " AS g" .
						   " WHERE ($children OR " . get_car_system_extension_goods($children) . ') AND g.goods_id = a.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 '.
						   " AND a.attr_id='$value' ".
					...
					$all_attr_list[$key]['attr_list'][0]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url, 'filter'=>$filter), $cat['cat_name']);
					...
					foreach ($attr_list as $k => $v)
					{
						...
						$all_attr_list[$key]['attr_list'][$temp_key]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url, 'filter'=>$filter), $cat['cat_name']);
			...
			foreach ($filter_attr AS $fkey=>$fval)
			{
				if ($fval)
				{
					...
					$condition[]=array(
							...
							'cond_name' =>$cond_name,
							'cond_url' =>build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url_68ecshop, 'filter'=>$filter), $cat['cat_name'])
						);
				}
			}
			$attr_group_more_txt = "";
		 ...
		 $filter_info = array(
			0=>array('id'=>0,'name'=>'全部','selected'=>0,'url'=>build_uri('car_system_category', array_merge($attr_url_value,array('filter'=>0)),'全部')),
			1=>array('id'=>1,'name'=>'网站自营','selected'=>0,'url'=>build_uri('car_system_category', array_merge($attr_url_value,array('filter'=>1)),'网站自营')),
			2=>array('id'=>2,'name'=>'入驻商店铺','selected'=>0,'url'=>build_uri('car_system_category', array_merge($attr_url_value,array('filter'=>2)),'入驻商店铺'))
		);
		...
		/*取得顶级ID*/
		$catlist = array();
		foreach(get_car_system_parent_cats($cat_id) as $k=>$v)
		{
			$catlist[] = $v['cat_id'];
		}

        //添加两分类
        $smarty->assign('categories_all',      get_categories_tree(0, 1)); // 分类树
        $smarty->assign('car_system_categories_all',      get_car_system_categories_tree(0, 1)); // 车系统分类树
        ...
        $smarty->assign('car_system_category',         $cat_id);
		...
		if ($brand > 0)
		{
			$arr['all'] = array('brand_id'  => 0,
							...
							'brand_logo'    => '',
							'goods_num'     => '',
							'url'           => build_uri('car_system_category', array('cid'=>$cat_id), $cat['cat_name'])
						);
		}
		else
		{
		...
		$brand_list = array_merge($arr, get_brands($cat_id, 'car_system_category'));
		...
		$smarty->assign('best_goods',      get_car_system_category_recommend_goods('best', $children, $brand, $price_min, $price_max, $ext));
		$smarty->assign('promotion_goods', get_car_system_category_recommend_goods('promote', $children, $brand, $price_min, $price_max, $ext));
		$smarty->assign('hot_goods',       get_car_system_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext));
	$smarty->assign('new_goods',       get_car_system_category_recommend_goods('new', $children, $brand, $price_min, $price_max, $ext));
		...
		$goodslist = car_system_category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order);
		if($display == 'grid')
		{
		...
		$smarty->assign('car_system_category',         $cat_id);
		$smarty->assign('script_name', 'car_system_category');
		...
		assign_pager('car_system_category',            $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $display, $filter_attr_str,'','',$filter); // 分页
		assign_dynamic('car_system_category'); // 动态内容
	}
...
function get_categories($cat_id = 0)
{
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
     */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('car_system_category') . " WHERE parent_id = '$cat_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, a.is_show,' .
            'b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order AS child_order ' .
            'FROM ' . $GLOBALS['ecs']->table('car_system_category') . ' AS a ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('car_system_category') . ' AS b ON b.parent_id = a.cat_id AND b.is_show = 1 ' .
    ...
    foreach ($res AS $row)
    {
        if ($row['is_show'])
        {
            ...
            $cat_arr[$row['cat_id']]['url']  = build_uri('car_system_category', array('cid' => $row['cat_id']), $row['cat_name']);

            if ($row['child_id'] != NULL)
            {
                $cat_arr[$row['cat_id']]['children'][$row['child_id']]['id']   = $row['child_id'];
                $cat_arr[$row['cat_id']]['children'][$row['child_id']]['name'] = $row['child_name'];
                $cat_arr[$row['cat_id']]['children'][$row['child_id']]['url']  = build_uri('car_system_category', array('cid' => $row['child_id']), $row['child_name']);
            }
        }
    }
...
{
    $sql = 'SELECT cat_name ' . 'FROM ' . $GLOBALS['ecs']->table('car_system_category')  . "WHERE cat_id =$id " ;
...
function get_parent($value,$id='')
{
    ...
    if($value!=0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('car_system_category') . " WHERE cat_id = '$value'";
...
{
    require_once ("themes/". $GLOBALS['_CFG']['template'] ."/library/lib_car_system_category_index.php" );

	$smarty->assign('cat_style',   htmlspecialchars($cat['style']));
}else{
	$smarty->assign('cat_style',   'category.css');
}
$smarty->assign('actname','car_system_category.php');
 ...
 * @return  void
 */
function get_car_system_cat_info($cat_id)
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('car_system_category') .
        " WHERE cat_id = '$cat_id'");
}
 ...
 * @return  array
 */
function car_system_category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
	$filter = (isset($_REQUEST['filter'])) ? intval($_REQUEST['filter']) : 0;
	
    $display = $GLOBALS['display'];
    $where = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND ".
            "g.is_delete = 0 AND ($children OR " . get_car_system_extension_goods($children) . ')';
...
{
	$filter = (isset($_REQUEST['filter'])) ? intval($_REQUEST['filter']) : 0;
    $where  = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_car_system_extension_goods($children) . ')';
 ...
 * @return int
 */
function get_car_system_parent_grade($cat_id)
{
    static $res = NULL;
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT parent_id, cat_id, grade ".
                   " FROM " . $GLOBALS['ecs']->table('car_system_category');
            $res = $GLOBALS['db']->getAll($sql);
