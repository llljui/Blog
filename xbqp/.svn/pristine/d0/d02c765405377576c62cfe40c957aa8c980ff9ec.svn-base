...
function get_car_system_cat_recommend_goods( $type = "", $cats = "", $cat_num = 0, $brand = 0, $min = 0, $max = 0, $ext = "" )
{
...
function get_car_system_child_cat( $tree_id = 0, $num = 0 )
{
		$three_arr = array( );
		...
		if ( $GLOBALS['db']->getOne( $sql ) || $tree_id == 0 )
		{
				$child_sql = "SELECT cat_id, cat_name, parent_id, is_show FROM ".$GLOBALS['ecs']->table( "car_system_category" ).( "WHERE parent_id = '".$tree_id."' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC {$where}" );
				$res = $GLOBALS['db']->getAll( $child_sql );
				foreach ( $res as $row )
				{
						if ( $row['is_show'] )
						{
								$three_arr[$row['cat_id']]['id'] = $row['cat_id'];
								$three_arr[$row['cat_id']]['name'] = $row['cat_name'];
								$three_arr[$row['cat_id']]['url'] = build_uri( "car_system_category", array(
										"cid" => $row['cat_id']
								), $row['cat_name'] );
						}
				}
...
function get_car_system_cat_brands( $cat, $num = 0, $app = "car_system_category" )
{
		$where = "";
		if ( $num != 0 )
		{
		...
		$children = 0 < $cat ? " AND ".get_car_system_children( $cat ) : "";
...
function get_car_system_hot_cat_tree( $pid = 0, $rec = 3 )
{
		$arr = array( );
		$sql = "select c.*, rc.recommend_type from ".$GLOBALS['ecs']->table( "car_system_category" )." as c left join ".$GLOBALS['ecs']->table( "car_system_cat_recommend" ).( " as rc on c.cat_id = rc.cat_id where c.parent_id=".$pid." order by c.sort_order asc, c.cat_id asc" );
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $row )
		{
				if ( $row['recommend_type'] == $rec )
				{
						$arr[$row['cat_id']]['id'] = $row['cat_id'];
						$arr[$row['cat_id']]['name'] = $row['cat_name'];
						$arr[$row['cat_id']]['url'] = build_uri( "car_system_category", array(
								"cid" => $row['cat_id']
						), $row['cat_name'] );
				}
		}
		foreach ( $res as $row )
		{
				$arr2 = get_car_system_hot_cat_tree2( $row['cat_id'], $rec );
				if ( $arr2 )
				{
...
function get_car_system_hot_cat_tree2( $pid = 0, $rec = 3 )
{
		$arr = array( );
		$sql = "select c.*, rc.recommend_type from ".$GLOBALS['ecs']->table( "car_system_category" )." as c left join ".$GLOBALS['ecs']->table( "car_system_cat_recommend" ).( " as rc on c.cat_id = rc.cat_id where rc.recommend_type=".$rec." and c.parent_id={$pid} order by c.sort_order asc, c.cat_id asc" );
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $row )
		{
				$arr[$row['cat_id']]['id'] = $row['cat_id'];
