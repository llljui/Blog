...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.show_in_index_cat, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
...
function cat_list1($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.show_in_index_cat,c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children,c.is_virtual ".
                'FROM ' . $GLOBALS['ecs']->table('category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('category') . " AS s ON s.parent_id=c.cat_id ".
...
/**
 * 过滤和排序所有分类，返回一个带有缩进级别的数组
 * added by wuzeguo car_system_cat
 *
 * @access  private
 * @param   int     $cat_id     上级分类ID
 * @param   array   $arr        含有所有分类的数组
 ...
 * @return  void
 */
function car_system_cat_options($spec_cat_id, $arr)
{
    static $cat_options = array();
    ...
    if (!isset($cat_options[0]))
    {
        $level = $last_cat_id = 0;
        $options = $cat_id_array = $level_array = array();
        $data = read_static_cache('car_system_cat_option_static');
        if ($data === false)
        {
            while (!empty($arr))
            {
            ...
            if (count($options) <= 2000)
            {
                write_static_cache('car_system_cat_option_static', $options);
            }
        }
        else
        {
...
//分类
function get_car_system_children($cat = 0,$ext='g')
{
    return $ext.'.system_cat_id ' . db_create_in(array_unique(array_merge(array($cat), array_keys(car_system_cat_list($cat, 0, false)))));
}
            ...
            else
            {
                if ($rewrite)
                {
                   $uri = 'car_system_category-' . $cid;
					...
					$uri = get_dir('car_system_category', $cid). '/'.$uri;		
					//$uri = 'category.php?id=' . $cid;
//                    if (!empty($bid))
//                    {
//                        $uri .= '&amp;brand=' . $bid;
...
//                    }
                ...
                {
                    $uri = 'car_system_category.php?id=' . $cid;
                    if (!empty($bid))
                    {
                        $uri .= '&amp;brand=' . $bid;
                    }
...
function get_dir($type, $id)
{
    ...
    elseif ($type == 'car_system_category')
    {
		$sql = "select path_name, cat_id from ". $GLOBALS['ecs']->table('car_system_category') ."  where cat_id= '$id' ";
		$path_row = $GLOBALS['db'] ->getRow($sql);
		$path_row['path_name'] = $path_row['path_name'] ? $path_row['path_name'] : ("cat".$path_row['cat_id']);
		$path_row['path_name'] = PREFIX_CATEGORY."-".$path_row['path_name'];
    }
...
if (!function_exists('debug')) {
    function debug($str, $debug='debug')
    {
        if (is_array($str)) $str = json_encode($str);
        include_once ROOT_PATH . '/includes/logger.php';
        logger::write($str,$debug);
    }
}

//定义商品年份，排量
function get_year_output()
{
    return array(
        'year_list' => array(
            '2000',
 ...
 * @return  mix
 */
function car_system_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;

    if ($res === NULL)
    {
        $data = read_static_cache('car_system_cat_pid_releate');
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.show_in_index_cat, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
            ...
            if (count($res) <= 1000)
            {
                write_static_cache('car_system_cat_pid_releate', $res);
            }
        }
        else
        {
    ...
    $options = car_system_cat_options($cat_id, $res); // 获得指定分类下的子分类的数组
...
function car_system_cat_list1($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;
...
{
    $ak = 'Ka72wIXXPTQQ5ntifS9bHwC6ZQnGA2xr';
    //http://lbsyun.baidu.com/index.php?title=webapi/guide/webservice-geocoding
    $url = "http://api.map.baidu.com/geocoder/v2/?address=$address&output=json&ak=$ak";
    $json = file_get_contents($url);
    $success = json_decode($json,true);
    if ($success['status'] == 0) {
        return $success['result']['location'];
    } else {
        return false;   
    }
}
