...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax') {
    ...
    $goodslist = car_system_category_get_goods($children, $other_has, $other_youhuo, $brand, $price_min, $price_max, '', $size, $page, $sort, $order, $limit);
...
if (!$smarty->is_cached('car_system_category.dwt', $cache_id)) {
    /* 如果页面没有被缓存则重新获取页面的内容 */
    ...
    if ($cat['grade'] > 1) {
        ...
        foreach ($price_grade as $key => $val) {
            ...
            $price_grade[$temp_key]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => $brand, 'price_min' => $price_grade[$temp_key]['start'], 'price_max' => $price_grade[$temp_key]['end'], 'filter_attr' => $filter_attr_str), $cat['cat_name']);
        ...
        $price_grade[0]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => $brand, 'price_min' => 0, 'price_max' => 0, 'filter_attr' => $filter_attr_str), $cat['cat_name']);
    ...
    foreach ($brands AS $key => $val) {
        ...
        $brands[$temp_key]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => $val['brand_id'], 'price_min' => $price_min, 'price_max' => $price_max, 'filter_attr' => $filter_attr_str), $cat['cat_name']);
    ...
    $brands[0]['brand_id'] = 0;
    $brands[0]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => 0, 'price_min' => $price_min, 'price_max' => $price_max, 'filter_attr' => $filter_attr_str), $cat['cat_name']);
    ...
    if ($cat['filter_attr'] > 0) {
        ...
        foreach ($cat_filter_attr AS $key => $value) {
            ...
            if ($temp_name = $db->getOne($sql)) {
                ...
                $all_attr_list[$key]['attr_list'][0]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => $brand, 'price_min' => $price_min, 'price_max' => $price_max, 'filter_attr' => $temp_arrt_url), $cat['cat_name']);
                ...
                foreach ($attr_list as $k => $v) {
                    ...
                    $all_attr_list[$key]['attr_list'][$temp_key]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => $brand, 'price_min' => $price_min, 'price_max' => $price_max, 'filter_attr' => $temp_arrt_url), $cat['cat_name']);
...
//LSJ End
    $smarty->assign('categories', get_car_system_categories_tree($cat_id)); // 分类树
    $smarty->assign('categories2', get_car_system_children_tree($cat_id));
    ...
    $goodslist = car_system_category_get_goods($children, $other_has, $other_youhuo, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order);
    if ($display == 'grid') {
    ...
    assign_pager('car_system_category', $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $display, $filter_attr_str); // 分页
...
$smarty->display('car_system_category.dwt', $cache_id);
 ...
 * @return  array
 */
function car_system_category_get_goods($children, $other_has, $other_youhuo, $brand, $min, $max, $ext, $size, $page, $sort, $order, $limit = '') {
