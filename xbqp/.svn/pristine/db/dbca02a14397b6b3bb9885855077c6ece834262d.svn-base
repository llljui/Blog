 ...
 * @return  array
 */
function get_categories_tree($cat_id = 0, $is_index=0)
{
    if ($cat_id > 0)
    {
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
                ...
                "WHERE parent_id = '$parent_id' AND is_show = 1 and is_virtual=0";
        $is_index && $sql .= ' AND show_in_index_cat = 1';
        $sql .= "  ORDER BY sort_order ASC, cat_id ASC";
...
//获取系统分类
//$is_index 是否首页
function get_car_system_categories_tree($cat_id = 0, $is_index=0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('car_system_category') . " WHERE cat_id = '$cat_id'";
        $parent_id = $GLOBALS['db']->getOne($sql);
    }
    else
    {
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('car_system_category') . " WHERE parent_id = '$parent_id' AND is_show = 1";
       ...
       $sql .= " ORDER BY sort_order ASC, cat_id ASC";
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_car_system_child_tree($row['cat_id']);
                }
            }
        }
    }
...
function get_car_system_child_tree($tree_id = 0)
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('car_system_category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('car_system_category') .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
               ...
               $three_arr[$row['cat_id']]['id']   = $row['cat_id'];
               $three_arr[$row['cat_id']]['name'] = $row['cat_name'];
               $three_arr[$row['cat_id']]['url']  = build_uri('car_system_category', array('cid' => $row['cat_id']), $row['cat_name']);

               if (isset($row['cat_id']) != NULL)
                   {
                       $three_arr[$row['cat_id']]['cat_id'] = get_car_system_child_tree($row['cat_id']);
 ...
 * @return  array
 */
function get_car_system_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    if (!empty($cats))
    {
        $sql .= " AND (" . $cats . " OR " . get_car_system_extension_goods($cats) .")";
    }
 ...
 * @return  array
 */
function assign_car_system_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['ecs']->table('car_system_category') . " WHERE cat_id = '$cat_id'";
    $cat['name'] = $GLOBALS['db']->getOne($sql);
    $cat['url']  = build_uri('car_system_category', array('cid' => $cat_id), $cat['name']);
    $cat['id']   = $cat_id;
...
//获取配件分类中的商品
function get_car_system_extension_goods($cats)
{
    $extension_goods_array = '';
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['ecs']->table('goods') . " AS g WHERE $cats";
    $extension_goods_array = $GLOBALS['db']->getCol($sql);
    return db_create_in($extension_goods_array, 'g.goods_id');
}
