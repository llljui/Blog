 ...
 * ============================================================================
 * $Author: Shadow & 小滨汽配
 * $Id: privilege.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
 */
define('IN_ECS', true);
...
/* 初始化 $exc 对象 */
$exc = new exchange($ecs->table("dept_admin_user"), $db, 'user_id', 'user_name');

// 定义条件中的大范围
$where_big = 'dept_id=' . $_SESSION['dept_id'];
...
function action_logout ()
{
	...
	/* 清除cookie */
	setcookie('ECSCP[dept_id]', '', 1);
	setcookie('ECSCP[dept_user_id]', '', 1);
	setcookie('ECSCP[dept_pass]', '', 1);
...
function action_signin ()
{
	...
	{
		$sql = "select user_name from " . $ecs->table('dept_admin_user') . " where email='" . $user_name . "'";
	...
	{
		$sql = "select user_name from " . $ecs->table('dept_admin_user') . " where mobile_phone='" . $user_name . "'";
		$rows = $db->query($sql);
		$i = 0;
	...
	$sql = "SELECT `ec_salt` FROM " . $ecs->table('dept_admin_user') . " WHERE user_name = '" . $user_name . "'";
	...
	{
		/* 检查密码是否正确 */
		$sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,dept_id,ec_salt" . " FROM " . $ecs->table('dept_admin_user') . " WHERE user_name = '" . $user_name . "' AND password = '" . md5(md5($_POST['password']) . $ec_salt) . "' AND checked=1";
	}
	else
	{
		/* 检查密码是否正确 */
		$sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,dept_id,ec_salt" . " FROM " . $ecs->table('dept_admin_user') . " WHERE user_name = '" . $user_name . "' AND password = '" . md5($_POST['password']) . "'  AND checked=1";
	}
    //echo $sql;die;
	$row = $db->getRow($sql);
	if($row)
	{
		...
		$_SESSION['dept_id'] = $row['dept_id']; // 店铺的id
		$_SESSION['dept_user_id'] = $row['user_id']; // 管理员id
		$_SESSION['dept_name'] = $row['user_name']; // 管理员名称
		$_SESSION['dept_action_list'] = $row['action_list']; // 管理员权限
		$_SESSION['dept_last_check'] = $row['last_login']; // 用于保存最后一次检查订单的时间
		...
		if(empty($row['ec_salt']))
		{
			...
			$db->query("UPDATE " . $ecs->table('dept_admin_user') . " SET ec_salt='" . $ec_salt . "', password='" . $new_possword . "'" . " WHERE user_id='$_SESSION[admin_id]'");
		}
		
		if($row['action_list'] == 'all')
		{
			$_SESSION['dept_admin_id'] = $row['user_id']; // 超级管理员的标识管理员id
			$_SESSION['dept_shop_guide'] = true; // 超级管理员标识
		}
		
		// 更新最后登录时间和IP
		$db->query("UPDATE " . $ecs->table('dept_admin_user') . " SET last_login='" . gmtime() . "', last_ip='" . real_ip() . "'" . " WHERE user_id='$_SESSION[dept_user_id]'");
		
		if(isset($_POST['remember']))
		{
			$time = gmtime() + 3600 * 24 * 365;
			setcookie('ECSCP[dept_id]', $row['dept_id'], $time);
			setcookie('ECSCP[dept_user_id]', $row['user_id'], $time);
			setcookie('ECSCP[dept_pass]', md5($new_possword . $_CFG['hash_code']), $time);
		}
	 ...
	 * {
	 * $dept_id = $db->getOne( "select dept_id from ".
	 * $ecs->table("dept") ." where status='1' and user_id=" .
	 * $row['user_id']);
	 * if (empty($dept_id))
	 * {
	 ...
	 * $_SESSION['dept_user_id'] = $row['user_id'];
	 * $_SESSION['dept_name'] = $row['user_name'];
	 *
	 *
	 * if (isset($_POST['remember']))
	 * {
	 * $time = gmtime() + 3600 * 24 * 365;
	 * setcookie('ECSCP[dept_id]', $dept_id, $time);
	 * setcookie('ECSCP[dept_user_id]', $row['user_id'], $time);
	 * setcookie('ECSCP[dept_pass]', md5($row['password'] .
	 * $_CFG['hash_code']), $time);
	 * }
	 *
	 * // 清除购物车中过期的数据
...
function action_insert ()
{
	...
	$sql = "SELECT nav_list FROM " . $ecs->table('dept_admin_user') . " WHERE action_list = 'all' AND dept_id=" . $_SESSION['dept_id'];
	$row = $db->getRow($sql);
	
	$sql = "INSERT INTO " . $ecs->table('dept_admin_user') . " (user_name, email, password, add_time, nav_list, action_list, role_id, dept_id, checked) " . "VALUES ('" . trim($_POST['user_name']) . "', '" . trim($_POST['email']) . "', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id', '$_SESSION[dept_id]', 1)";
...
function action_edit ()
{
	...
	/* 查看是否有权限编辑其他管理员的信息 */
	if($_SESSION['dept_user_id'] != $_REQUEST['id'])
	{
		admin_priv('admin_manage');
	}
	
	/* 获取管理员信息 */
	$sql = "SELECT user_id, user_name, email, password, agency_id, role_id, dept_id FROM " . $ecs->table('dept_admin_user') . " WHERE user_id = '" . $_REQUEST['id'] . "' AND dept_id=" . $_SESSION['dept_id'];
	...
	/* 获得该管理员的权限 */
	$priv_str = $db->getOne("SELECT action_list FROM " . $ecs->table('dept_admin_user') . " WHERE user_id = '$_GET[id]' AND dept_id = '$_SESSION[dept_id]'");
...
function action_update ()
{
	...
	else
	{
		...
		$admin_id = $_SESSION['dept_user_id'];
	...
	{
		/* 查询旧密码并与输入的旧密码比较是否相同 */
		$sql = "SELECT password FROM " . $ecs->table('dept_admin_user') . " WHERE user_id = '$admin_id' AND dept_id=" . $_SESSION['dept_id'];
		$old_password = $db->getOne($sql);
		$sql = "SELECT ec_salt FROM " . $ecs->table('dept_admin_user') . " WHERE user_id = '$admin_id' AND dept_id=" . $_SESSION['dept_id'];
	...
	if($pwd_modified)
	{
		$sql = "UPDATE " . $ecs->table('dept_admin_user') . " SET " . "user_name = '$admin_name', " . "email = '$admin_email', " . "ec_salt = '$ec_salt' " . $action_list . $role_id . $password . $nav_list . "WHERE user_id = '$admin_id' AND dept_id=" . $_SESSION['dept_id'];
	}
	else
	{
		$sql = "UPDATE " . $ecs->table('dept_admin_user') . " SET " . "user_name = '$admin_name', " . "email = '$admin_email' " . $action_list . $role_id . $nav_list . "WHERE user_id = '$admin_id' AND dept_id=" . $_SESSION['dept_id'];
	}
...
function action_modif ()
{
	...
	/* 获得当前管理员数据信息 */
	$sql = "SELECT u.user_id, u.user_name, u.email, s.nav_list " . "FROM " . $ecs->table('users') . " AS u, " . $ecs->table('dept') . "AS s WHERE u.user_id=s.user_id AND u.user_id = '" . $_SESSION['dept_user_id'] . "'";
	$user_info = $db->getRow($sql);
...
function action_allot ()
{
	...
	include_once (ROOT_PATH . 'languages/' . $_CFG['lang'] . '/admin/dept_priv_action.php');
	
	admin_priv('allot_priv');
	if($_SESSION['dept_user_id'] == $_GET['id'])
	{
	...
	/* 获取权限的分组数据 */
	$sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " . $ecs->table('dept_admin_action') . " WHERE parent_id = 0";
	...
	/* 按权限组查询底级的权限名称 */
	$sql = "SELECT action_id, parent_id, action_code,relevance FROM " . $ecs->table('dept_admin_action') . " WHERE parent_id " . db_create_in(array_keys($priv_arr));
	$result = $db->query($sql);
...
function action_update_allot ()
{
	...
	/* 取得当前管理员用户名 */
	$admin_name = $db->getOne("SELECT user_name FROM " . $ecs->table('dept_admin_user') . " WHERE user_id = '$_POST[id]' AND dept_id = '$_SESSION[dept_id]'");
	
	/* 更新管理员的权限 */
	$act_list = @join(",", $_POST['action_code']);
	$sql = "UPDATE " . $ecs->table('dept_admin_user') . " SET action_list = '$act_list', role_id = '' " . "WHERE user_id = '$_POST[id]' AND dept_id = '$_SESSION[dept_id]'";
...
function action_remove ()
{
	...
	/* 获得管理员用户名 */
	$action_list = $db->getOne('SELECT action_list FROM ' . $ecs->table('dept_admin_user') . " WHERE user_id='$id' AND dept_id='$_SESSION[dept_id]'");
...
{
	$list = array();
	$sql = 'SELECT user_id, user_name, email, add_time, last_login, dept_id ' . 'FROM ' . $GLOBALS['ecs']->table('dept_admin_user') . ' where checked=1 and dept_id = ' . $_SESSION['dept_id'] . ' ORDER BY user_id DESC';
