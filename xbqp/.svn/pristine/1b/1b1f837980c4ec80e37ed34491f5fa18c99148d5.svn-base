...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT MAX(order_id) FROM " . $ecs->table('order_info') . " as o WHERE dept_id = '$order[dept_id]' and order_id < '$order[order_id]'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $ecs->table('order_info') . " as o WHERE dept_id = '$order[dept_id]' and order_id > '$order[order_id]'";
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.dept_ids, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            /* 发放红包 */
            send_order_bonus($order_id,$order['dept_id']);
        }
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('shipping' == $step)
    {
        ...
        $shipping_list = available_shipping_list($region_id_list,$order['dept_id']);
    ...
    elseif ('money' == $step)
    {
        ...
        if ($order['user_id'] > 0)
        {
			...
			/* 取得用户可用红包 */
            $user_bonus = user_bonus($order['user_id'], array($order['dept_id']=>$order['goods_amount']));
			if (!empty($user_bonus)){
				$user_bonus1 = $user_bonus[$order['dept_id']];
...
{   
    ...
    $sql = "SELECT o.order_sn, u.user_name " .
            ...
            "WHERE o.user_id > 0 AND o.dept_id='". $_SESSION['dept_id'] ."' " .
...
/*------------------------------------------------------ */
   ...
   $msg = '';
        ...
        /* 获取当前操作员 */
        $delivery['action_user'] = $_SESSION['dept_name'];
 	...
 	define('GMTIME_UTC', gmtime()); 
        ...
        $filter_fileds = array(
                               ...
                               'dept_id', 'status', 'order_id', 'shipping_name'
                               );
        $_delivery = array();
    ...
    /* 发货单发货记录log edited by wuzeguo*/
    order_action($order['order_sn'], OS_CONFIRMED, $shipping_status, $order['pay_status'], $action_note, $_SESSION['dept_name'], 1);
    /* 如果当前订单已经全部发货 */
    if ($order_finish)
    {
        /* 如果订单用户不为空，计算积分，并发给用户；发红包 */
...
/*------------------------------------------------------ */
    ...
    elseif (isset($_POST['ship']))
    {
        ...
        $depts_list = get_depts_list();
        $depts_list_count = count($depts_list);
        $smarty->assign('depts_name', depts_list_name()); // 取供货商名
        $smarty->assign('depts_list', ($depts_list_count == 0 ? 0 : $depts_list)); // 取供货商列表
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    if ('confirm' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_CONFIRMED, SS_UNSHIPPED, PS_UNPAYED, $action_note, $_SESSION['dept_name']);
    ...
    elseif ('pay' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_CONFIRMED, $order['shipping_status'], PS_PAYED, $action_note , $_SESSION['dept_name']);
    }
    /* 设为未付款 */
    ...
    elseif ('prepare' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_CONFIRMED, SS_PREPARING, $order['pay_status'], $action_note, $_SESSION['dept_name']);
    ...
    elseif ('split' == $operation)
    {
        ...
        /* 获取表单提交数据 */
        $dept_id = isset($_REQUEST['dept_id']) ? intval(trim($_REQUEST['dept_id'])) : '0';
        ...
        /* 记录log */
        order_action($order['order_sn'], $arr['order_status'], $shipping_status, $order['pay_status'], $action_note, $_SESSION['dept_name']);
    ...
    elseif ('unship' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], $order['order_status'], SS_UNSHIPPED, $order['pay_status'], $action_note, $_SESSION['dept_name']);
    ...
    elseif ('receive' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, $order['pay_status'], $action_note, $_SESSION['dept_name']);
    }
    /* 取消 */
    elseif ('cancel' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_CANCELED, $order['shipping_status'], PS_UNPAYED, $action_note, $_SESSION['dept_name']);
    ...
    elseif ('invalid' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_INVALID, $order['shipping_status'], PS_UNPAYED, $action_note, $_SESSION['dept_name']);
    ...
    elseif ('return' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_RETURNED, SS_UNSHIPPED, PS_UNPAYED, $action_note, $_SESSION['dept_name']);
    ...
    {
        /* 记录log */
        order_action($order['order_sn'], $order['order_status'], $order['shipping_status'], $order['pay_status'], '[' . $_LANG['op_after_service'] . '] ' . $action_note, $_SESSION['dept_name']);
    }
    else
    {
        die('invalid params');
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        /*增值税发票_添加_END_bbs.hongyuvip.com*/
        $where = "WHERE o.dept_id='".$_SESSION['dept_id']."' ";
        if ($filter['order_sn'])
        {
...
function get_order_goods($order)
{
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, g.dept_ids AS dept_id,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn " .
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($admin_info['dept_id'] > 0)
        {
            $where .= " AND d.dept_id = '" . $admin_info['dept_id'] . "' ";
        }
        ...
        $sql = "SELECT d.delivery_id, d.delivery_sn, d.order_sn, d.order_id, d.add_time, d.action_user, d.consignee, d.country,
                       d.province, d.city, d.district, d.tel, d.status, d.update_time, d.email, d.depts_id
    ...
    $depts_list = get_depts_list();
    $_depts_list = array();
    foreach ($depts_list as $value)
    {
        $_depts_list[$value['depts_id']] = $value['depts_name'];
    }
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['depts_name'] = isset($_depts_list[$value['depts_id']]) ? $_depts_list[$value['depts_id']] : '';
    }
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($admin_info['dept_id'] > 0)
        {
            $where .= " AND b.dept_id = '" . $admin_info['dept_id'] . "' ";
        }
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    if ($admin_info['dept_id'] > 0)
    {
        $where .= " AND dept_id = '" . $admin_info['dept_id'] . "' ";
    }
