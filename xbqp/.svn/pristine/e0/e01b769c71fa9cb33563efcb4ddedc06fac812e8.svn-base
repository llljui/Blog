 ...
 * ============================================================================
 * $Author: Shadow & 小滨汽配
 * $Id: car_system_search.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
 */
...
if (empty($_GET['encode']))
{
    ...
    header("Location: car_system_search.php?encode=$string\n");
...
if ($_REQUEST['act'] == 'advanced_search')
{
    ...
    $smarty->display('car_system_search.dwt');
...
else
{
    ...
    if($_CFG['fulltext_search'] == '0'){
        ...
        if($count==0)
        {
            if (preg_match('/^[a-zA-z]+$/i', $_REQUEST['keywords']))
            {
                ...
                {
                    header('Location: car_system_search.php?keywords='.$keyword_www_68ecshop_com.'&keyword_zm='.$_REQUEST['keywords']);
                    exit;
                }
            }
        }
        ...
        if($result){
            ...
            if($count==0)
            {
                if (preg_match('/^[a-zA-z]+$/i', $_REQUEST['keywords']))
                {
                    ...
                    {
                        header('Location: car_system_search.php?keywords='.$keyword.'&keyword_zm='.$_REQUEST['keywords']);
                        exit;
                    }
                }
            }
    ...
    /* 分页 */
    $url_format = "car_system_search.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" . $_REQUEST['brand']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
    if (!empty($intromode))
    {
    ...
    $pager = get_pager('car_system_search.php', $pager['search'], $count, $page, $size);
    ...
    foreach ($kcat_list as $kkey=>$kkcat)
    {
        ...
        if(!empty($cat_arr)){
            ...
            foreach($cat_arr as $val){
                ...
                $cat_two_arr[$cat_arr[$anum]['cat_id']][$cat_arr[0]['cat_id']]['url'] = 'car_system_search.php?category='. $val['cat_id'] .'&keywords='.$Recordkw;
    ...
    if (strlen($Recordkw)>1)
    {
        ...
        foreach($cat_two_arr as $key => $key_rrr){
            ...
            $url = 'car_system_search.php?category='. $key .'&keywords='.$Recordkw;
