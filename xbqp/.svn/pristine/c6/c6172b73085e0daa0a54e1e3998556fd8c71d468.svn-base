 ...
 * ---------------------------------------------
 * $Author: Shadow & 小滨汽配
 * $Id: template.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
*/
...
if ($_REQUEST['act'] == 'setup')
{
    ...
    foreach ($temp_libs AS $val)
    {
        ...
        if ($val['lib'] == 'cat_goods')
        {
            ...
            {
                $cate_goods[] = array('region' => $val['region'], 'sort_order' => $val['sort_order'], 'number' => $row['number'], 'cat_id' => $row['id'], 'cats'=>car_system_cat_list(0, $row['id']));
            }
            else
            {
                $cate_goods[] = array('region' => $val['region'], 'sort_order' => $val['sort_order'], 'number'=>0, 'cats'=>car_system_cat_list(0));
            }
        }
    ...
    $smarty->assign('arr_cates',          car_system_cat_list(0, 0, true));
