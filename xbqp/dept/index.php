 ...
 * ============================================================================
 * $Author: Shadow & 小滨汽配
 * $Id: index.php 17217 2016-01-19 06:29:08Z Shadow & 小滨汽配
*/
...
elseif ($_REQUEST['act'] == 'top')
{
    // 获得管理员设置的菜单
    $lst = array();
    //$nav = $db->GetOne('SELECT nav_list FROM ' . $ecs->table('dept') . " WHERE user_id = '" . $_SESSION['dept_user_id'] . "'");
...
elseif ($_REQUEST['act'] == 'main')
{
	...
	$smarty->assign('dept_article',  $dept_article);
    $smarty->assign('dept_notice',  nl2br($_CFG['dept_notice']));
     */
    ...
    $today = array();
    //今日订单总额
    $today['money'] = $db->GetOne('SELECT SUM(money_paid) AS money FROM ' .$ecs->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.' AND dept_id='.$_SESSION['dept_id'].order_query_sql('finished'));
    $today['money'] = price_format($today['money']);
    //今日订单数
    $today['order'] = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info'). ' WHERE `confirm_time` BETWEEN '.$today_start.' AND '.$today_end.' AND dept_id='.$_SESSION['dept_id']);
    $smarty->assign('today',$today);
    //待处理事务
    $task = array();
    //待审核商品
    //$task['goods'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('goods').' WHERE dept_status=0 AND dept_id='.$_SESSION['dept_id']);
    //待处理佣金
    //$task['commission'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('dept_rebate').' WHERE is_pay_ok=0 '.' AND dept_id='.$_SESSION['dept_id']);
    //未查看平台文章
    //$sql = "SELECT cat_id FROM ". $ecs->table('article_cat') ." WHERE cat_type='99' ";
	//$article_cat_id = $db->getOne($sql);
    //$task['article'] = $db->getOne("SELECT COUNT(*)  FROM ". $ecs->table('dept_article') ." WHERE dept_id = ".$_SESSION['dept_id']);
    ...
    //商品相关
    //商品总数
    $goods['total'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND dept_id='.$_SESSION['dept_id']);
    //审核通过商品数
    $goods['pass'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND dept_status=1 AND dept_id='.$_SESSION['dept_id']);
    //审核未通过商品数
    $goods['impass'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND dept_status=-1 AND dept_id='.$_SESSION['dept_id']);
    //库存警告
    if ($_CFG['use_storage'])
    {
        $sql = 'SELECT COUNT(*) FROM ' .$ecs->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real = 1 AND dept_id='.$_SESSION['dept_id'];
    ...
    ' WHERE is_delete = 0 AND is_new = 1 AND is_real = 1 AND dept_id='.$_SESSION['dept_id']);
    //精品推荐总数
    $goods['best']    = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
    ' WHERE is_delete = 0 AND is_best = 1 AND is_real = 1 AND dept_id='.$_SESSION['dept_id']);
    //热销商品总数
    $goods['hot']     = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
    ' WHERE is_delete = 0 AND is_hot = 1 AND is_real = 1 AND dept_id='.$_SESSION['dept_id']);
    //促销商品总数
    $time             = gmtime();
    $goods['promote'] = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
    ' WHERE is_delete = 0 AND promote_price>0' .
    ' AND promote_start_date <= '.$time.' AND promote_end_date >= '.$time.' AND is_real = 1 AND dept_id='.$_SESSION['dept_id']);
    //已下架商品总数
    $goods['deleted'] = $db->getOne(' SELECT COUNT(*) FROM '.$ecs->table('goods').' WHERE is_on_sale = 0 AND is_real=1 AND is_delete=0 AND is_alone_sale = 1 AND dept_id='.$_SESSION['dept_id']);
    $smarty->assign('goods',$goods);
     */
    
    //订单相关
    //订单总数
    $order['total'] = $db->GetOne('SELECT COUNT(*) FROM '.$ecs->table('order_info'). ' WHERE 1'.' AND dept_id='.$_SESSION['dept_id']);
    //待发货订单
    $order['await_ship']   = $db->GetOne('SELECT COUNT(*)'.
    ' FROM ' .$ecs->table('order_info') .
    " WHERE 1 " . order_query_sql('await_ship').' AND dept_id='.$_SESSION['dept_id']);
    $status['await_ship']  = CS_AWAIT_SHIP;
    //待支付订单
    $order['await_pay']    = $db->GetOne('SELECT COUNT(*)'.
    ' FROM ' .$ecs->table('order_info') .
    " WHERE 1 " . order_query_sql('await_pay').' AND dept_id='.$_SESSION['dept_id']);
    $status['await_pay']   = CS_AWAIT_PAY;
    //待确认订单
    $order['unconfirmed']  = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info').
    " WHERE 1 " . order_query_sql('unconfirmed').' AND dept_id='.$_SESSION['dept_id']);
    $status['unconfirmed'] = OS_UNCONFIRMED;
    //部分发货的订单
    $order['shipped_part']  = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info').
    " WHERE  shipping_status=" .SS_SHIPPED_PART.' AND dept_id='.$_SESSION['dept_id']);
    $status['shipped_part'] = OS_SHIPPED_PART;

	//退款申请
	$sql = 'SELECT COUNT(*) FROM ' . $ecs->table('back_order') . ' WHERE status_back=5 AND dept_id="'.$_SESSION['dept_id'].'"';
    $order['new_repay'] = $db->getOne($sql );
    //退货申请
    $order['returns'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back=5 AND dept_id="'.$_SESSION['dept_id'].'"');
    //缺货登记
    $order['booking_goods'] = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('booking_goods') . ' AS bg LEFT JOIN '.$ecs->table('goods').' AS g ON bg.goods_id=g.goods_id WHERE FIND_IN_SET ('.$_SESSION['dept_id'].', g.dept_ids) AND is_dispose=0');

   
    //已完成订单总数
    $order['finished'] = $db->GetOne('SELECT COUNT(*) FROM '.$ecs->table('order_info'). ' WHERE 1'.order_query_sql('finished').' AND dept_id='.$_SESSION['dept_id']);
    ...
    $sql = 'SELECT `froms`, count(*) AS `count` FROM '.$ecs->table('order_info').' WHERE `confirm_time` BETWEEN '.$month_start.' AND '.$month_end. ' AND dept_id='.$_SESSION['dept_id'].' GROUP BY `froms` ORDER BY `count` DESC';
    $result = $db->query($sql);
    ...
    $result = $db->query('SELECT DATE_FORMAT(FROM_UNIXTIME(`confirm_time`),"%d") AS day,COUNT(*) AS count,SUM(money_paid) AS money FROM '.$ecs->table('order_info').' WHERE `confirm_time` BETWEEN '.$month_start.' AND '.$month_end.' '.order_query_sql('finished').'  AND dept_id='.$_SESSION['dept_id'].' GROUP BY day ORDER BY day ASC ');
...
elseif ($_REQUEST['act'] == 'check_order')
{
	...
	$where_suppId = " AND dept_id = '" . $_SESSION['dept_id'] . "'";
    /* 新订单 */
